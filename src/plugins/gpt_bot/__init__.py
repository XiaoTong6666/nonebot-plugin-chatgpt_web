...
def chushihua_liulanqi() -> bool:
    """同步初始化浏览器"""
    global liulanqi, biaoqian, yichushihua
    try:
        logger.info("正在初始化 ChatGPT 浏览器...")
...
def tiwen_gpt(wenti: str) -> Optional[str]:
    """同步提问 ChatGPT"""
    if not yichushihua or not biaoqian:
        raise Exception("GPT 未初始化")

    try:
        logger.info(f"提问: {wenti}")
        wenti_anquan = wenti.replace("\\", "\\\\").replace("`", "\\`").replace("${", "\\${")

        fasong_js = f"""
        (async () => {{
            const bianjiqi = document.querySelector('#prompt-textarea');
            if (!bianjiqi) throw new Error("找不到输入框 #prompt-textarea");
            bianjiqi.innerText = `{wenti_anquan}`;
        ...
        while time.time() - start < peizhi.response_timeout:
            ...
            if neirong and neirong != zuihou_neirong:
                ...
                while time.time() - stable_start < 2:
                    time.sleep(0.2)
                    current = biaoqian.run_js("return window.zuihouHuifu || '';")
                    if current != stable:
                        stable = current
                        stable_start = time.time()
...
async def kaishi_gpt():
    ...
    with concurrent.futures.ThreadPoolExecutor() as pool:
        success = await loop.run_in_executor(pool, chushihua_liulanqi)
...
@gpt.handle()
async def gpt_mingling(matcher: Matcher, bot: Bot, event: Event, args: Message = CommandArg()):
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        with concurrent.futures.ThreadPoolExecutor() as pool:
            huida = await loop.run_in_executor(pool, tiwen_gpt, wenti)
